...
const listener = (request, sender, sendResponse) => {
  ...
  switch (request.message) {
    ...
    case "retrievePrefs": {
      StorageHelper.retrievePrefs(request.action).then((res) =>
        sendResponse(res)
      )
      return true
      break
...
const commandListener = async (command) => {
  ...
  if (command === "toggle-bionic") {
    ...
    const [{ data: tabBrMode }, getReadingModeError] = await new Promise(
      (res, _) => {
    ...
    chrome.tabs.sendMessage(
      activeTab.id,
      { type: "setReadingMode", data: intentedTabBrMode },
      () => Logger.LogLastError()
    )
