...
const IndexContent = () => {
    ...
    const getCollapseExpandBtn = () => <button
        onClick={toggleExpandeHandler}> {isExpanded ? "Collapse" : "Expand"}</button>;
					...
					{isExpanded &&
                        ...
                        Object.entries(prefs).map(([key, val], index) => (
                            <p className="prefsEntry" key={"prefs_item" + index}>
                                ...
                                <span
                                    className="prefsValue">{typeof val == "boolean" ? (val == true ? "true" : "false") : val}</span>
                            </p>
                        ))}
				</span>
                {getCollapseExpandBtn()}
